    ...
    switch (msg.type) {
        ...
        case 'custom':
            ...
            if(content.type===1){
                ...
                str = '<img class="chartlet" onload="loadImg()" src="/webim/im/images/'+catalog+'/' +chartlet+'.png">';
            }else if(content.type==4){
...
function getAvatar(url){
    ...
    if(re.test(url)){
        ...
        return "/webim/im/images/default-icon.png"
    }
}   
...
function getNick(account,cache){
    ...
    return cache.getFriendAlias(account)||((cache.getUserById(account) || cache.getUserById(account) =="")?cache.getUserById(account).nick:window.getNick(account));
}
