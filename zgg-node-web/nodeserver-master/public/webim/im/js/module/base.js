...
YX.fn.showMe = function () {
	...
	//如果客户没有昵称默认显示果蜜
	if(user.nick == 'gm' || user.nick == ''){
		this.$userName.text('果蜜')
		this.$userPic.attr('src', getAvatar(user.avatar))
		setCookie('nickName','果蜜')
...
YX.fn.openChatBox = function (account, scene) {
    ...
    if(scene=="p2p"){
        ...
        if(info.account == userUID){
            ...
            this.$chatTitle.find('img').attr('src', "/webim/im/images/myPhone.png")
        }else{
        ...
        if(info){
            if(info.avatar){
                ...
                this.$chatTitle.find('img').attr('src', "/webim/im/images/" + info.type + ".png")
            }
            ...
            this.$chatTitle.find('img').attr('src', "/webim/im/images/normal.png")
    ...
    switch(type){
        case "session":   
            ...
            if(scene==="p2p"){
                ...
                if(msg.target === userUID){
                    info.nick = "我的手机"
                    info.avatar = "/webim/im/images/myPhone.png"
                }else{
                ...
                if(teamInfo){
                    ...
                    if(teamInfo.avatar){
                        ...
                        info.avatar = "/webim/im/images/"+teamInfo.type+".png"
                    }
                }else{
                    info.nick = msg.target
                    info.avatar = "/webim/im/images/normal.png"
                }   
            }
        break
        case "friend":
        ...
        case "team":
            ...
            if(data.avatar){
                ...
                info.avatar = info.type==="normal"?"/webim/im/images/normal.png":"/webim/im/images/advanced.png"
            }
            info.crtSession = this.crtSession  
        break
    }
...
YX.fn.doLogout = function () {
    delCookie('uid')
    delCookie('sdktoken')
    window.location.href = 'http://www.zhiguoguo.com'
}
