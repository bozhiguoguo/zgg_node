...
function wrapListener(agent, listener) {
    ...
    return tracer.actionProxy(function wrappedHandler(request, response) {
        var action;
        if (!agent.config.enabled || !(action = tracer.getAction())) {
            return listener.apply(this, arguments);
        }
        var isXmlHttpRequest = (function(headers) {
            return headers && headers['x-requested-with'];
        })(request.headers);
        ...
        response._tingyun = {
            ...
            getJsTag: function(agent, resAction) {
                ...
                var mixed = agent.config.rum['mix_enabled'];
                logger.debug('nbs.rum.mix_enabled: %s', mixed);
                if (mixed) {
                    if (response.headersSent) {
                        logger.debug('header has been sent, can not set header!');
                    } else {
                        response.setHeader("Set-Cookie", 'TINGYUN_DATA=' + encodeURIComponent(agentConf));
                    }
                    return null;
                }
...
function injectJSTag(htmlSegment, jsTag, res) {
    var headTagIndex, headTag, _tingyun = res._tingyun,
        limit;
