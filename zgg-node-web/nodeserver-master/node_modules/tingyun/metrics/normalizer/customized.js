...
function WebActionNaming(req, actionNamingRules) {
    if (!actionNamingRules || !actionNamingRules.length) {
        throw new Error('Web action naming rule is empty.');
    }
    this.req = req;
...
proto.match = function() {
    ...
    for (var i = 0, length = rules.length; i < length; i++) {
        ...
        if (!this.methodMatch(rule)) {
            logger.debug('Rule (%s): method does not match, current method is %s, rule method id is %s', rule.name, this.req.method, rule.match.method);
            continue;
        };
        if (!this.parsed) {
            this.parse();
...
proto.methodMatch = function(rule) {
    ...
    return method == 'ALL' || method == this.req.method ? true : false;
};
...
proto.parse = function() {
    ...
    this.body = toLowerCase(req.body);
